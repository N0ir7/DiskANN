...
template<typename T, typename TagT>
void LevelNMerger<T, TagT>::process_inserts_pq() {
...
template<typename T, typename TagT>
void LevelNMerger<T, TagT>::process_inserts() {
  Timer total_insert_timer;
...
template<typename T, typename TagT>
void LevelNMerger<T, TagT>::insert_disk_vec(diskann::DiskNode<T> & node) {
  Timer timer;
  float insert_time, delta_time;
  // START: mem_vec has no ID, no presence in system
  std::vector<diskann::Neighbor>         pool;
  std::vector<diskann::Neighbor>         tmp;
  tsl::robin_map<uint32_t, T *> coord_map;
  const T* vec = node.coords;
  const uint32_t offset_id = this->offset_ids[0] + disk_node.id;
  // search on combined graph
  this->offset_iterate_to_fixed_point(vec, this->l_index, pool,
                                      coord_map);
  ...
  // get renamed ID
  const uint32_t renamed_id = this->rename(offset_id);
  assert(renamed_id != std::numeric_limits<uint32_t>::max());

  // insert backward edge
  this->disk_delta->inter_insert(renamed_id, new_nhood.data(),
                                  (_u32) new_nhood.size());

  // insert into graph
  node.nnbrs = (_u32) new_nhood.size();
  *(node.nbrs - 1) = (_u32) nhood.size();

  memcpy(node.nbrs, new_nhood.data(), node.nnbrs * sizeof(uint32_t));
  // insert pq data

  // compute PQ coords
  std::vector<uint8_t> pq_coords = this->disk_index->deflate_vector(vec);

  // directly copy into PQFlashIndex PQ data
  const uint64_t pq_offset = (uint64_t) renamed_id * (uint64_t) this->pq_nchunks;
  memcpy(this->pq_data + pq_offset, pq_coords.data(),
          this->pq_nchunks * sizeof(uint8_t));
...
template<typename T, typename TagT>
void LevelNMerger<T, TagT>::offset_iterate_to_fixed_point(
...
template<typename T, typename TagT>
void LevelNMerger<T, TagT>::prune_neighbors(
...
template<typename T, typename TagT>
void LevelNMerger<T, TagT>::prune_neighbors_pq(
    std::vector<diskann::Neighbor> &pool, std::vector<uint32_t> &pruned_list,
    uint8_t *scratch) {
  if (pool.size() == 0)
    return;
...
template<typename T, typename TagT>
void LevelNMerger<T, TagT>::occlude_list(
    std::vector<diskann::Neighbor> &              pool,
    const tsl::robin_map<uint32_t, T *> &coord_map,
...
template<typename T, typename TagT>
void LevelNMerger<T, TagT>::occlude_list_pq(
...
template<typename T, typename TagT>
void LevelNMerger<T, TagT>::dump_to_disk(const uint32_t start_id,
                                            const char *   buf,
...
// template<typename T, typename TagT>
// void LevelNMerger<T, TagT>::compute_deleted_ids() {
...
template<typename T, typename TagT>
void LevelNMerger<T, TagT>::process_deletes() {
...
// template<typename T, typename TagT>
// void LevelNMerger<T, TagT>::populate_deleted_nhoods() {
//   // buf for scratch
//   char *buf = nullptr;
//   alloc_aligned((void **) &buf, SECTORS_PER_MERGE * SECTOR_LEN, SECTOR_LEN);
...
template<typename T, typename TagT>
void LevelNMerger<T, TagT>::consolidate_deletes(diskann::DiskNode<T> &disk_node,
                                                    uint8_t *    scratch) {
  // if node is deleted
...
// template<typename T, typename TagT>
// bool LevelNMerger<T, TagT>::is_deleted(const diskann::DiskNode<T> &disk_node) {
...
// TODO
template<typename T, typename TagT>
void LevelNMerger<T, TagT>::compute_rename_map() {
  uint32_t needed = 0;
...
template<typename T, typename TagT>
uint32_t LevelNMerger<T, TagT>::rename(uint32_t id) const {
...
template<typename T, typename TagT>
uint32_t LevelNMerger<T, TagT>::rename_inverse(uint32_t renamed_id) const {
  auto iter = std::lower_bound(
...
template<typename T, typename TagT>
void LevelNMerger<T, TagT>::rename(diskann::DiskNode<T> &node) const {
...
template<typename T, typename TagT>
void LevelNMerger<T, TagT>::rename(std::vector<uint32_t> &ids) const {
...
template<typename T, typename TagT>
uint32_t LevelNMerger<T, TagT>::get_index_id(
...
template<typename T, typename TagT>
std::vector<uint32_t> LevelNMerger<T, TagT>::get_edge_list(
...
template<typename T, typename TagT>
const T *LevelNMerger<T, TagT>::get_mem_data(const uint32_t offset_id) {
  const uint32_t index_no = this->get_index_id(offset_id);
  if (index_no == std::numeric_limits<uint32_t>::max()) {
    assert(offset_id < this->offset_ids[0]);
    return nullptr;
...
template<typename T, typename TagT>
void LevelNMerger<T, TagT>::write_tag_file(
...
template<typename T, typename TagT>
void LevelNMerger<T, TagT>::process_merges() {
  // buf to hold data being read
  char *buf = nullptr;
  alloc_aligned((void **) &buf, SECTORS_PER_MERGE * SECTOR_LEN, SECTOR_LEN);
...
void LevelNMerger<T, TagT>::InitIndexPaths(const char * dist_disk_index_path,
  ...
  // this->disk_index_in_path = to_disk_in; // 输入磁盘索引的路径
  ...
  from_disk_index_merger_ = DiskIndexMerger<T,TagT>(DiskIndexFileMeta(from_disk_path,single_file_index_));
  to_disk_index_merger_ = DiskIndexMerger<T,TagT>(DiskIndexFileMeta(dist_disk_index_path,single_file_index_))
  ...
  //     this->single_file_index_ ? this->disk_index_out_path
  //                               : this->disk_index_out_path + "_disk.index";
  // this->final_pq_coords_file =
  //     this->single_file_index_
...
void LevelNMerger<T, TagT>::merge(const char * dist_disk_index_path,
  ...
  // this->from_disk_index_merger_.InitGraphDelta(base_offset);
  this->offset_ids.push_back(base_offset);
...
void LevelNMerger<T, TagT>::DeletePhase(){
  ...
  this->disk_deleted_nhoods = this->to_disk_index_merger_.PopulateNondeletedHoodsOfDeletedNodes();

  // process all deletes
  this->to_disk_index_merger_.ProcessDeletes(this->intermediate_index_file_meta_,
                                            this->disk_deleted_nhoods,
                                            this->thread_bufs); 
  // END -- graph on disk has NO deleted references, maybe some holes
}
template<typename T, typename TagT>
void LevelNMerger<T, TagT>::InsertPhase(){
  DiskIndexDataIterator<T, TagT> from_disk_index_data_iter = this->from_disk_index_merger_.GetIterator();
  DiskIndexDataIterator<T, TagT> to_disk_index_data_iter = this->to_disk_index_merger_.GetIterator();
  ...
  while (from_disk_index_data_iter.HasNextBatch()){
    ...
    #pragma omp parallel for schedule(dynamic, 128) num_threads(MAX_N_THREADS)
    for(int i = 0; i < from_node_batch->size(); i++){
      diskann::DiskNode<T>& from_node = (*from_node_batch)[i];
      diskann::DiskNode<T>& from_node_tag = tag_list[i];
      // diskann::Timer timer;
      // float insert_time, delta_time;
      std::vector<diskann::Neighbor> pool;
      tsl::robin_map<uint32_t, T *> coord_map;
      uint32_t       omp_thread_no = omp_get_thread_num();
      this->to_disk_index_merger_.OffsetIterateToFixedPoint(vec, this->l_index, pool,
                                        coord_map, thread_data);
      // insert_time = (float) timer.elapsed();

      // prune neighbors using alpha
      // TODO: 将原本的邻居也加入到pool中
      std::vector<uint32_t> new_nhood;
      this->to_disk_index_merger_.PruneNeighbors(coord_map, pool, new_nhood);

      // add backward edge to disk index delta
      this->to_disk_index_merger_.delta.inter_insert(offset_id, new_nhood.data(),
                                    (_u32) new_nhood.size());
      ...
      #pragma omp critical
      while(to_disk_index_data_iter.HasNext()){
        std::tie(to_node, to_node_pq_coord, to_node_tag) = to_disk_index_data_iter.Next();
        if(!this->to_disk_index_merger_.IsFree(to_node->id)){ // If not free, skip
          continue;
        }
        break;
      }
      ...
      if(to_node && to_node_pq_coord && to_node_tag){ // If find the location
        // copy vector
        memcpy(to_node.coords, from_node.coords, this->from_disk_index_merger_.ndim() * sizeof(T));
        // number of nhoods
        to_node.nnbrs =  new_nhood.nnbrs;
        // copy new nhoods
         ...
         */
        // compute PQ coords
        std::vector<uint8_t> new_pq_coords = this->to_disk_index_merger_.DeflateVector(to_node.coords);

        // directly copy into PQFlashIndex PQ data
        memcpy(to_node_pq_coord, new_pq_coords.data(),
                this->to_disk_index_merger_.pq_nchunks() * sizeof(uint8_t));
        /**
         * insert tag data
        */
        ...
        to_disk_index_data_iter.NotifyFlushBack();
      }
      // delta_time = (float) timer.elapsed();
...
void LevelNMerger<T, TagT>::PatchPhase(){
  DiskIndexDataIterator<T, TagT> to_disk_index_data_iter(this->to_disk_index_merger_.meta.data_path,
                                                          this->to_disk_index_merger_.index);
  to_disk_index_data_iter.Init(false/* read_write*/,this->final_index_file_meta_.data_path);
  
  while(to_disk_index_data_iter.HasNextBatch()){
    std::vector<diskann::DiskNode<T>>* from_node_batch = nullptr;
    std::tie(from_node_batch, std::ignore, std::ignore) = to_disk_index_data_iter.NextBatch();

    #pragma omp parallel for schedule(dynamic, 128) num_threads(MAX_N_THREADS)
    for(int i = 0; i < from_node_batch->size(); i++){

      diskann::DiskNode<T>& =disk_node = (*from_node_batch)[i];
      int      omp_thread_no = omp_get_thread_num();
      uint8_t *thread_scratch = this->thread_bufs[omp_thread_no];

      // get backward edge set
      std::vector<uint32_t> deltas = this->to_disk_index_merger_.delta->get_nhood(disk_node.id);
      if(deltas.empty()){
        continue;
      }
      ...
      if (nhood.size() > this->range) {
        ...
        this->disk_index->compute_pq_dists(disk_node.id, nhood.data(), dists.data(),
                                            (_u32) nhood.size(),
                                            thread_scratch);
        for (uint32_t k = 0; k < nhood.size(); k++) {
          pool[k].id = nhood[k];
        ...
        nhood.clear();
        this->to_disk_index_merger_.PruneNeighborsWithPQDistance(pool, nhood, thread_scratch);
      }
      // copy edges from nhood to disk node
      disk_node.nnbrs = (_u32) nhood.size();
      *(disk_node.nbrs - 1) = (_u32) nhood.size();  // write to buf
      memcpy(disk_node.nbrs, nhood.data(),
              disk_node.nnbrs * sizeof(uint32_t));
      // notify the disk index to flush back the insertion
      to_disk_index_data_iter.NotifyNodeFlushBack();
    }
  }
}
template<typename T, typename TagT>
 ...
 */
InsertPhase();
// std::fstream new_data_writer(this->temp_disk_index_path,
//                         std::ios::out | std::ios::binary);
// assert(new_data_writer.is_open());
// new_data_writer.seekp(0, std::ios::end);
// // buf to hold data being read
// char *buf = nullptr;
// alloc_aligned((void **) &buf, SECTORS_PER_MERGE * SECTOR_LEN, SECTOR_LEN);
// std::vector<diskann::DiskNode<T>> disk_nodes;
...
// while (start_id < this->from_disk_npts) {
//   // 读取SECTORS_PER_MERGE个扇区的数据
//   new_start_id = this->from_disk_index->merge_read(disk_nodes, start_id,
//                                               SECTORS_PER_MERGE, buf);
//   diskann::Timer timer;
//   #pragma omp parallel for schedule(dynamic, 1) num_threads(MAX_INSERT_THREADS)
//   for (int64_t i = 0; i < (int64_t) disk_nodes.size(); i++) {
//     diskann::DiskNode<T> &disk_node = disk_nodes[i];
//     const tsl::robin_set<uint32_t> &deleted_set = this->disk_deleted_ids;
//     if (deleted_set.find((uint32_t) j) != deleted_set.end()) {
//       disk_node.nnbrs = 0;
//       *(disk_node.nbrs - 1) = (_u32)0;
//       continue;
//     }
//     if (((i % 100000) == 0) && (i > 0)) {
//       diskann::cout << "Finished inserting " << i << " points" << std::endl;
//       std::cout << "When i = " << i
//                 << " elapsed time: " << timer.elapsed() / 1000000 << "s"
//                 << std::endl;
//     }
//     // insert into index
//     this->insert_disk_vec(disk_node);
//   }
//   // advance to next block
//   new_data_writer.write(buf, SECTORS_PER_MERGE * SECTOR_LEN);
//   disk_nodes.clear();
//   id_nhoods.clear();
//   start_id = new_start_id;
// }
...
// double   delta_time = std::accumulate(this->delta_times.begin(),
//                                     this->delta_times.end(), 0.0);
// uint32_t n_inserts = this->from_disk_npts;
// diskann::cout << "TIMER:: Insert time per point = "
//               << insert_time / n_inserts
//               << ", Delta = " << delta_time / n_inserts << "\n";
// diskann::cout << " E2E insert time: " << e2e_time << " sec" << std::endl;
...
uint32_t LevelNMerger<T, TagT>::ComputeNewMaxPts(){
  ...
  if (needed > this->to_disk_index_merger_.free_ids.size()) {
...
void LevelNMerger<T, TagT>::WriteIntermediateIndexFile(DiskIndexFileMeta& src_index_file_meta,
  ...
  tag_writer.write((char *) &npts_u32, sizeof(int));
  tag_writer.write((char *) &ndims_u32, sizeof(int));
  tag_writer.close();
