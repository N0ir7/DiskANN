...
#pragma omp parallel for schedule(dynamic, 128) num_threads(MAX_N_THREADS)
    for (int64_t idx = 0; idx < (int64_t) disk_nodes.size(); idx++) {
      ...
      diskann::DiskNode<T> &disk_node = disk_nodes[idx];
...
void Level0Merger<T, TagT>::merge(const char *                    disk_in,
  ...
  this->disk_delta = new diskann::GraphDelta(0, this->disk_npts);
  ...
  for (auto &mem_index_path : mem_in) {
    ...
    if (!(this->_single_file_index)) {
      ...
      this->mem_deltas.push_back(new diskann::GraphDelta(index_offset, npts));
